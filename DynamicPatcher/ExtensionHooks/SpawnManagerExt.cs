...
namespace ExtensionHooks
{
    public class SpawnManagerExtHooks
    {
        ...
        public static unsafe UInt32 SpawnManagerClass_Update_Add_Missile_Target(REGISTERS* R)
        {
            ...
            // 如果目标在天上，则自动开启跟踪模式
            if (!pTarget.IsNull && pTarget.Ref.IsInAir() && pRocket.TryGetComponent<MissileHomingScript>(out MissileHomingScript homing))
            {
        ...
        public static unsafe UInt32 KamikazeTrackerClass_Cannot_Detach2(REGISTERS* R)
        {
            ...
            if (pRocket.IsNull || pRocket.Convert<ObjectClass>().IsDeadOrInvisible())
            {
                // 导弹作为目标时，死亡的是导弹，也是目标
            ...
            if (pTarget.IsNull || !pTarget.CastToTechno(out Pointer<TechnoClass> pTehcno) || pTehcno.IsDeadOrInvisible())
            {
                // Logger.Log($"{Game.CurrentFrame} - KamikazeTrackerClass_Cannot_Detach2 导弹 {pKamikaze.Ref.Aircraft} 的目标是个死人");
                // 换掉目标
                CoordStruct lastLocation = pRocket.Ref.Base.Base.Base.Base.GetCoords();
                if (pRocket.Convert<TechnoClass>().TryGetComponent<MissileHomingScript>(out MissileHomingScript homing))
                {
        ...
        public static unsafe UInt32 RocketLocomotionClass_Process_Step5_To_Lazy_4(REGISTERS* R)
        {
            ...
            if (pFoot.Convert<TechnoClass>().TryGetComponent<MissileHomingScript>(out MissileHomingScript homing) && homing.IsHoming)
            {
                return 0x662A32;
            }
            return 0;
        ...
        public static unsafe UInt32 RocketLocomotionClass_663030(REGISTERS* R)
        {
            try
            {
                ...
                // Logger.Log($"{Game.CurrentFrame} - 子机导弹 {pTechno} [{pTechno.Ref.Type.Ref.Base.Base.ID}] 爆炸");
