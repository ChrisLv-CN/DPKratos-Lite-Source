    ...
    public class ArcingTrajectoryScript : BulletScriptable
    {
        ...
        public override void OnUpdate()
        {
            ...
            if (trajectoryData.AdvancedBallistics && !pBullet.Ref.WH.HasPreImpactAnim())
            {
                ...
                if (double.IsNaN(dist) || double.IsInfinity(dist) || dist < 64 + pBullet.Ref.Type.Ref.Acceleration)
                {
        ...
        public void ResetVelocity(float speedMultiple = 1f)
        {
            if (!InitFlag)
            {
                ...
                BulletVelocity velocity = WeaponHelper.GetBulletArcingVelocity(sourcePos, targetPos, speed, gravity, lobber, inaccurate, min, max, out double straightDistance, out double realSpeed);
